    ...
    function plgVmConfirmedOrder($cart, $order) {
      ...
      $i = 0;
        ...
        if ($method->direct_checkout == 1) {
            $html.= '<input type="submit" id="tco_submit" value="' . JText::_('VMPAYMENT_TCO_BUTTON_MESSAGE') . '" />';
            $html.= '</form></div>';
            $html.= '<p>' . JText::_('VMPAYMENT_TCO_REDIRECT_MESSAGE') . '<p>';
            $html.= '<script src="https://www.2checkout.com/static/checkout/javascript/direct.min.js"></script>';
            $html.= '<script>
                        function waitForTco()
                        {
                            if (typeof inline_2Checkout != "undefined")
                            {
                                window.setTimeout("jQuery(\'#tco_submit\').click();",100);
                            }
                            else
                            {
                                window.setTimeout("waitForTco();",100);
                            }
                        }
                        waitForTco();
                    </script>';
